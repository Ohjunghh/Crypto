...
void print_state(unsigned char *state){
    for(int i=0;i<4;i++){//0->4->8->12->1>5>9>13
        for(int j=0;j<4;j++){
...
void shift_rows(unsigned char *msg){
    ...
    tmp=msg[3];tmp2=msg[7];
    msg[3]=msg[15];msg[7]=tmp;msg[15]=msg[11];msg[11]=tmp2;
}
void inv_shift_rows(unsigned char *msg){
    unsigned char tmp,tmp2;
    tmp=msg[5];
    msg[5]=msg[1]; msg[1]=msg[13];  msg[13]=msg[9]; msg[9]=tmp; 
    tmp=msg[10];tmp2=msg[14];
    msg[10]=msg[2]; msg[14]=msg[6]; msg[2]=tmp; msg[6]=tmp2;
    tmp=msg[15];
    msg[15]=msg[3];msg[3]=msg[7];msg[7]=msg[11];msg[11]=tmp;
}
...
void mix_columns(unsigned char *msg){
   unsigned char mix[4][4]=
    ...
    for(int y=0;y<4;y++){ //mix 행
        for(int x=0;x<4;x++){ //msg 열
            for(int i=0;i<4;i++){
                d=msg[4*x+i];
                switch(mix[y][i]){
                    case 1: c[i]=d; break;
...
void key_schedule(unsigned char *key,unsigned char round_key[ROUND+1][16]){ // 라운드키 만드는 함수
