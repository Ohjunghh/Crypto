...
unsigned char doub(unsigned char c){
    bool left_most_bit=c&1<<7; //0&1=0, 1&1=1; c&(1<<7)임
    c=c<<1;
    if(left_most_bit)
        c=c^0x1b;
    return c;
...
void AES_encrypt(unsigned char *msg,unsigned char *key,unsigned char *ct){
    ...
    key_schedule(key,round_key);//라운드키 채우기
    add_round_key(ct,round_key,0);//0번째 라운드키 사용

    for(int i=1;i<ROUND;i++){ //1~9라운드
        sub_bytes(ct);
        shift_rows(ct);
        mix_columns(ct);
        ...
        sub_bytes(ct);
        shift_rows(ct);
        add_round_key(ct,round_key,ROUND);//10라운드
}
void AES_decrypt(unsigned char *ct,unsigned char *key, unsigned char *msg){
    ...
    key_schedule(key,round_key);//라운드키 채우기
    add_round_key(msg,round_key,ROUND);//0번째 라운드키 사용

    for(int i=ROUND-1;i>0;i--){ //1~9라운드
        inv_shift_rows(msg);
        inv_sub_bytes(msg);
        ...
        inv_shift_rows(msg);
        inv_sub_bytes(msg);
        add_round_key(msg,round_key,0);//10라운드
}
...
int main()
{
    ...
    for(int i=1;i<ROUND+1;i++){
        for(int j=0;j<16;j++){
            cout << uppercase <<hex << (int)round_key[i][j] << " ";
        }
        cout << endl;
    }*/
    unsigned char msg[16]={0x04,0x00,0x12,0x18,0x02,0x11,0x18,0x0F,0x13,0x0E,0x06,0x11,0x00,0x0F,0x07,0x18};
    unsigned char key[16]={0x54,0x68,0x61,0x74,0x73,0x20,0x6D,0x79,0x20,0x4B,0x75,0x6E,0x67,0x20,0x46,0x75};
    unsigned char ct[16];
